...
public class PlanillaEmpleado extends javax.swing.JPanel {
    ...
    private void LlenarComboBoxCargo() {
        DefaultComboBoxModel<String> value;
        ...
        if (Conexion.isModoPrueba()) {
            // Agregar datos de ejemplo para visualización del frontend
            value = new DefaultComboBoxModel<>();
            cbCargo.setModel(value);
            
            // Datos de ejemplo para cargos
            String[] cargosEjemplo = {"Desarrollador Senior", "Gerente de Proyecto", "Analista de Sistemas", "Contador General"};
            double[] salariosEjemplo = {2500.00, 4500.00, 1800.00, 3200.00};
            double[] diasEjemplo = {30.0, 30.0, 30.0, 30.0};
            double[] horasEjemplo = {160.0, 160.0, 160.0, 160.0};
            double[] recargosEjemplo = {0.10, 0.15, 0.05, 0.12};
            double[] isssEjemplo = {0.03, 0.03, 0.03, 0.03};
            double[] afpEjemplo = {0.0725, 0.0725, 0.0725, 0.0725};
            double[] incafEjemplo = {0.01, 0.01, 0.01, 0.01};
            
            for (int i = 0; i < cargosEjemplo.length; i++) {
                String nombreCargo = cargosEjemplo[i];
                double salarioNominal = salariosEjemplo[i];
                double dias = diasEjemplo[i];
                double horas = horasEjemplo[i];
                double recargo = recargosEjemplo[i];
                double isss = isssEjemplo[i];
                double afp = afpEjemplo[i];
                double incaf = incafEjemplo[i];
            ...
            System.out.println("Modo prueba: Cargando datos de ejemplo para cargos");
            return;
        }
        ...
        try {
            ...
            Connection conn = this.connect.getConexion();
            if (conn == null) {
                System.out.println("Error: No hay conexión disponible a la base de datos");
                return;
            }
            PreparedStatement sentencia1 = conn.prepareCall(sentencia);
...
public void actualizarTabla(JTable tabla) {
       ...
       if (Conexion.isModoPrueba()) {
//           // Agregar datos de ejemplo para visualización del frontend
//           modelo.addRow(new Object[]{"12345678-9", "Desarrollador Senior", "Juan", "Pérez", 12, 300.00, 500.00, "3%", "7.25%", "1%", 2500.00, "$2,500.00"});
//           modelo.addRow(new Object[]{"98765432-1", "Gerente de Proyecto", "María", "González", 12, 300.00, 500.00, "3%", "7.25%", "1%", 4500.00, "$4,500.00"});
//           modelo.addRow(new Object[]{"45678912-3", "Analista de Sistemas", "Carlos", "Martínez", 6, 150.00, 250.00, "3%", "7.25%", "1%", 1800.00, "$1,800.00"});
//           modelo.addRow(new Object[]{"78912345-6", "Contador General", "Ana", "Rodríguez", 12, 300.00, 500.00, "3%", "7.25%", "1%", 3200.00, "$3,200.00"});
           System.out.println("Modo prueba: Mostrando datos de ejemplo en la planilla de empleados");
           return;
       }
       ...
       try {
           ...
           sentencia1 = conn.prepareCall(sentencia);
           ResultSet rs = sentencia1.executeQuery();
           while (rs.next()) {
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        ...
        if (Conexion.isModoPrueba()) {
            JOptionPane.showMessageDialog(this,
                "Modo prueba: La eliminación de empleados no está disponible sin conexión a la base de datos.",
                "Modo Prueba",
        ...
        if (confirmacion == JOptionPane.YES_OPTION) {
            try {
                ...
                Connection conn = this.connect.getConexion();
                if (conn == null) {
                    JOptionPane.showMessageDialog(this, "Error: No hay conexión disponible a la base de datos", "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }
    ...
    private void btnAgregarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAgregarActionPerformed
        ...
        if (nombres.isEmpty() || cantMeses==0 || apellidos.isEmpty() || tipoCargo.isEmpty() || duiStr.isEmpty()) {
            ...
            if (Conexion.isModoPrueba()) {
                JOptionPane.showMessageDialog(this,
                    "Modo prueba: El registro de empleados no está disponible sin conexión a la base de datos.",
                    "Modo Prueba",
                    JOptionPane.INFORMATION_MESSAGE);
                return;
            }
            ...
            try {
                ...
                PreparedStatement psConsulta = conn.prepareStatement(consultaDui);
